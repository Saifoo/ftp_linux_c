...
#define LISTEN_ENQ 5 /* listen backlog */
#define MAX_RECV_BUF 256
#define MAX_SEND_BUF 256
#define FILE_PATH "~/Downloads"
...
int main(int argc, char* argv[])
{
	...
	for( ; ; ) /* infinite loop */
	{
		...
		/* accept is a blocking call, it will wait until a client connects */
		...
		/* hence loop continues so the server can accept multiple client requests */
	}	/* end for loop */
...
void get_file_name(int sock, char* file_name)
{
	...
	printf("\nfile_name is found to be: %s\n", file_name); /* test printing */

}

int send_file(int sock, char *file_name)
{
	int sent_count; /* how many sending chunks, for debugging purpose*/
